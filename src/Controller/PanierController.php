...
class PanierController extends AbstractController
{
    ...
    public function show(SessionInterface $session, ProduitRepository $repo): Response
    {
        ...
        /*
         ...
         - dans le tableau dataPanier je rajoute à chaque tour de boucle un nouveau tableau qui contient une clé "produit" avec comme valeur le produit récuperé, et une autre "quantite" qui contient la quantite du produit en question
        - puis à chaque tour de boucle je calcule le prix total du produit (prix du produit * quantite) et je l'ajoute à ma variable $total 
        */
    ...
    public function add($id, SessionInterface $session)
    {
        ...
        // si il n'existe pas la valeur est un tableau vide []
        $panier = $session->get('panier', []);

        if (empty($panier[$id])) {
           $panier[$id] = 1; // si $id n'existe pas, j'ajoute 1
        }else{
            $panier[$id]++; // si il existe, je rajoute +1
        }
